...
func _update_camera(delta, speed_ratio):
	...
	rig.position.x = lerp(rig.position.x, steer_smoothed * 1.45, 1.0 - exp(-delta * 4.0))
	rig.position.y = lerp(rig.position.y, 3.0 + speed_ratio * 0.58 + shake, 1.0 - exp(-delta * 2.5))
	rig.position.z = lerp(rig.position.z, 7.3 + speed_ratio * 2.15, 1.0 - exp(-delta * 2.2))
